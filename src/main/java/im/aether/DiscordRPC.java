...
public class DiscordRPC {
    ...
    public static Pair<File, File> downloadDiscordLibrary() {
        ...
        try {
            URLConnection connection = new URL("https://dl-game-sdk.discordapp.net/2.5.6/discord_game_sdk.zip").openConnection();
            ...
            while ((entry = zin.getNextEntry()) != null) {
                if (entry.getName().equals(zipPath)) {
                    ...
                    File tempDir = new File(System.getProperty("java.io.tmpdir"), "ae-rpc-" + name + System.nanoTime());
                    if (!tempDir.mkdir())
    ...
    public static void initActivity(final String appId, final boolean activeTime) {
        ...
        try (CreateParams params = new CreateParams()) {
            ...
            activity.setState("Made by github.com/imAETHER");
            ...
            if (activeTime) {
                ...
                activity.timestamps().setStart(new Date(bootMillis).toInstant());
            }

            // Loading image, this will only show up for a few seconds
            activity.assets().setLargeImage("https://cdn.discordapp.com/emojis/924458990069497897.gif");
     ...
     * otherwise you can use links to an image/gif. The link length is limited to 127, otherwise an exception will be thrown.
     *
    ...
    public static void update(final String details, final String state, final String large, final String small) {
        if (core == null || activity == null) return;
        boolean updateDetails = !activity.getDetails().equals(details) && details.length() >= 2,
                updateState = !activity.getState().equals(state) && state.length() >= 2,
                updateLarge = !activity.assets().getLargeText().equals(large),
                updateSmall = !activity.assets().getSmallText().equals(small);
