...
public class Main {
	...
	public static void main(String[] args) throws Exception {
		...
		final String cfgData = Files.readAllLines(CONFIG.toPath(), StandardCharsets.UTF_8)
		                            .stream()
		                            .reduce("", (a, b) -> a + b);
		...
		// temp(libs) and tempDir(lib directory), this is so Core doesn't create more temp dirs and just uses the
		// given one
	...
	private static Pair<String, String> getGPUInfoNvidia() {
		try {
			final Process gpuTemp = Runtime.getRuntime()
			                               .exec(new String[]{"cmd", "/c", "nvidia-smi --query-gpu=temperature.gpu " +
					                               "--format=csv,noheader"});
			gpuTemp.waitFor();
			...
			final Process gpuUsage = Runtime.getRuntime()
			                                .exec(new String[]{"cmd", "/c", "nvidia-smi --query-gpu=utilization.gpu " +
					                                "--format=csv,noheader"});
			gpuUsage.waitFor();
			//read the output from the command
